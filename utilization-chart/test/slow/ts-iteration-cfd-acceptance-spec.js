describe("When using an extended model and setting cfd", function() {
    ...
    it("should populate __acceptance with a total plan estimate each day ignoring wrong days",function(){
        runs(function(){
        ...
        runs (function(){
            ...
            expect(iteration.get('__dailyAcceptance').length).toEqual(2);
            expect(iteration.get('__dailyAcceptance')).toEqual([3,4]);
            expect(iteration.get('__endAcceptance')).toEqual(4);
            
        });
    });
    
    it("should populate __acceptance with a total plan estimate each day with nulls for dates that have no data",function(){
        runs(function(){
            Rally.technicalservices.ModelBuilder.build('Iteration','tsIteration',[]).then({
                success: function(result) {
                    model = result;
        ...
        runs (function(){
            ...
            expect(iteration.get('__dailyAcceptance').length).toEqual(3);
            expect(iteration.get('__dailyAcceptance')).toEqual([3,4,null]);
            expect(iteration.get('__endAcceptance')).toEqual(null);
        });
    });  
    
    it("should populate __acceptance with a total plan estimate each day",function(){
        runs(function(){
            Rally.technicalservices.ModelBuilder.build('Iteration','tsIteration',[]).then({
                success: function(result) {
                    model = result;
        ...
        runs (function(){
            ...
            expect(iteration.get('__dailyAcceptance').length).toEqual(1);
            expect(iteration.get('__dailyAcceptance')).toEqual([3]);
            expect(iteration.get('__endAcceptance')).toEqual(3);
        });
    });
    
    it("should populate __acceptance with a total plan estimate each day ignoring CFD from other iterations",function(){
        runs(function(){
            Rally.technicalservices.ModelBuilder.build('Iteration','tsIteration',[]).then({
                success: function(result) {
                    model = result;
    ...
    it("should deal with __acceptance when there is no data yet",function(){
        runs(function(){
            Rally.technicalservices.ModelBuilder.build('Iteration','tsIteration',[]).then({
                success: function(result) {
                    model = result;
        ...
        runs (function(){
            expect(iteration.get('__days').length).toEqual(1);
            expect(iteration.get('__dailyAcceptance').length).toEqual(1);
            expect(iteration.get('__dailyAcceptance')).toEqual([null]);
            expect(iteration.get('__endAcceptance')).toEqual(null);
        });
